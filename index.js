...
app.get("/userdata/:email",async (req,res)=>{
  ...
  console.log(user);
  res.status(201).json({message :"data fetched successfully",userdata:user});
  }catch{
...
app.patch('/updatedata/:email',async (req,res)=>{
  ...
  try{
    const user=await Register.updateOne({email:req.params.email},{name:newname});
    const userdata=await Register.findOne({email:req.params.email});
    res.status(201).json({message:"user data update succesfully",userdata : userdata});
  }catch{
    res.status(500).json({message:"user not found"});
  }
})
...
app.delete('/deleteuser/:email',async (req,res)=>{
  const usertest=await Register.findOne({email:req.params.email});
  if(!usertest){
    res.status(500).json({message:"user not found"});
  }
  try{
    const user=await Register.deleteOne({email:req.params.email});
